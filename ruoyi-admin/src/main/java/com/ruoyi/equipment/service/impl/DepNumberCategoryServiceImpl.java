...
public class DepNumberCategoryServiceImpl implements IDepNumberCategoryService 
{
    ...
    protected final static Logger log = LoggerFactory.getLogger(DepNumberCategoryServiceImpl.class);
     ...
     * 查询分行
     *
     * @param depNumberCategory
     * @return 分行数量集合
     */
    @Override
    public List<DepNumberCategory> selectBranch(DepNumberCategory depNumberCategory) {
        List<DepNumberCategory> depNumberCategories = depNumberCategoryMapper.selectBranch(depNumberCategory);
        DepNumberCategory result = new DepNumberCategory();
        result.setBranchId("");
        result.setBranchName("全部");
        depNumberCategories.add(0,result);
    ...
    @Override
    public Map<String,Object> insertCategory(String parentId, String name) {
        int i = 0;
        Map<String, Object> result = new HashMap<>();
        String[] names = name.split(",");
        log.info("AAAAAAAAAAA"+name);
        log.info("bbbbbbbbbbb"+names.toString());
        for (String brand : names) {
            log.info("CCCCCCCCCCC"+brand);

            //获取编号
            Map<String, Object> map = depNumberCategoryMapper.getNextIdentity("");
            ...
            i = depNumberCategoryMapper.insertCategory(identity, brand, parentId);
            if(i==0){
                result.put("code","2");
                result.put("msg","添加"+brand+"设备失败");
                return result;
            }
        }
        result.put("code","0");
    ...
    @Override
    public int deleteFeedback(String seqNo) {
        return depNumberCategoryMapper.deleteFeedback(seqNo);
    }
}
