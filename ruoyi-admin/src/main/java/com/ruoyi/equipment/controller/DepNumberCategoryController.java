...
public class DepNumberCategoryController extends BaseController
{
    ...
    public ModelMap list(@RequestBody String json)
    {
        ...
        branchId = note.getString("branchId");
        if (StringUtils.isNotBlank(pageNum) && StringUtils.isNotBlank(pageSize)) {
            ...
            depNumberCategory.setBranchId(branchId);
            if(!user_id.equals("")){
    ...
    @ResponseBody
    public TableDataInfo selectBranch(@RequestBody String json)
    {
        JSONObject jsonObject = JSON.parseObject(json);
        JSONObject note = (JSONObject) jsonObject.get("note");
        ...
        DepNumberCategory depNumberCategory = new DepNumberCategory();
        if(!depMap.get("ORG_CODE").toString().equals("1000100")){
            depNumberCategory.setBranchId(depMap.get("ORG_CODE").toString());
        }
        List<DepNumberCategory> depNumberCategories = depNumberCategoryService.selectBranch(depNumberCategory);
        return getDataTable(depNumberCategories);
    }
    ...
    @ResponseBody
    public ModelMap insertCategory(@RequestBody String json)
    {
        ModelMap model = new ModelMap();
        log.info("bbbbbbbbbb"+json);
        JSONObject jsonObject = JSON.parseObject(json);
        JSONObject note = (JSONObject) jsonObject.get("note");
        String parentId = note.getString("parentId");
        String name = note.getString("name");
        Map<String, Object> result = depNumberCategoryService.insertCategory(parentId, name);
        if("2".equals(result.get("code").toString())){
            model.put("code","0");
            model.put("msg",result.get("msg"));
        }else{
            model.put("code","1");
            model.put("msg","添加设备成功");
        }
        return model;
    }
    ...
    @ResponseBody
    public ModelMap deleteFeedback(@RequestBody String json)
    {
        ModelMap model = new ModelMap();
        JSONObject jsonObject = JSON.parseObject(json);
        JSONObject note = (JSONObject) jsonObject.get("note");
        String seqNo = note.getString("seqNo");
        int result = depNumberCategoryService.deleteFeedback(seqNo);
        if(result>0){
            model.put("code","0");
            model.put("msg","删除成功");
        }else{
