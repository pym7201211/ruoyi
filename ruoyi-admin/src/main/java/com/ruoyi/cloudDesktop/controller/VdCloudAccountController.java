...
public class VdCloudAccountController extends BaseController
{
    ...
    @ResponseBody
    public ModelMap listSearch(@RequestBody String json)
    {
        ModelMap model = new ModelMap();
        JSONObject jsonObject = JSON.parseObject(json);
        JSONObject note = (JSONObject) jsonObject.get("note");
        ...
        VdCloudAccount vdCloudAccount = new VdCloudAccount();
        vdCloudAccount.setAccount(account);
        List<VdCloudAccount> teams = vdCloudAccountService.selectTeamAccount(null);
        List<VdCloudAccount> accounts = vdCloudAccountService.selectVdCloudAccountList(vdCloudAccount);
        model.put("team",teams);
        model.put("account",accounts);
